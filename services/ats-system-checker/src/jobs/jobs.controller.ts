...
export class JobsController {
  ...
  async getAll(@Payload() payload: GetAllJobsDto) {
    return this.jobsService.getAllJobs(payload?.user_id, !!payload?.include_inactive);
  }

  @MessagePattern('ats.jobs.create')
  async create(@Payload() body: CreateJobDto) {
    const userId = body?.user_id;
    if (!userId) throw new BadRequestException({ success: false, error: true, message: 'user_id is required' });
    if (userId.length !== 24 || !/^[0-9a-f]{24}$/i.test(userId)) {
      throw new BadRequestException({ success: false, error: true, message: 'Invalid user_id format' });
    }
  ...
  async getOne(@Payload() payload: JobIdDto) {
    if (!payload?.job_id?.trim()) throw new BadRequestException('job_id is required');
  ...
  async update(@Payload() payload: UpdateJobDto) {
    if (!payload?.job_id?.trim()) throw new BadRequestException('job_id is required');
    return this.jobsService.updateJob(payload.job_id, payload.updates, payload.user_id);
  }
  ...
  async apply(@Payload() payload: ApplyJobDto) {
    ...
    if (!payload?.candidate_email?.trim()) {
      throw new BadRequestException({ success: false, error: true, message: 'candidate_email is required' });
    }
    if (!payload?.file?.filename?.trim() || !payload?.file?.data_base64?.trim()) {
      throw new BadRequestException({ success: false, error: true, message: 'file (filename, data_base64) is required' });
    }
