...
function extractS3Key(cvFilename: string): string {
  ...
  if (cvFilename.startsWith('http')) {
    if (cvFilename.includes('/s3/file/')) return cvFilename.split('/s3/file/')[1];
    if (cvFilename.includes('cv_uploads/')) return cvFilename.match(/cv_uploads\/[^/]+$/)?.[0] || cvFilename;
  }
...
export class ApplicationsController {
  ...
  async getAll(@Payload() payload: GetAllApplicationsDto) {
    ...
    if (effective && (effective.length !== 24 || !/^[0-9a-f]{24}$/i.test(effective))) {
      throw new BadRequestException('Invalid user_id format');
    }
    const applications = await this.db.getAllApplications(effective);
    return {
  ...
  async getOne(@Payload() payload: GetOneApplicationDto) {
    ...
    if (userId) {
      const app = await this.db.getApplicationById(appId);
      if (!app) throw new NotFoundException('Application not found');
      const job = await this.db.getJobPostingById(app.job_id);
      if (!job || job.owner_user_id !== userId) throw new ForbiddenException('You can only view applications for your own job postings');
    }
    const application = await this.db.getApplicationById(appId);
    if (!application) throw new NotFoundException('Application not found');
    let quizScore = application.quiz_score;
    if (quizScore == null) {
      const quizResult = await this.db.getQuizResultByApplicationId(application.application_id);
  ...
  async update(@Payload() payload: UpdateApplicationDto) {
    ...
    if (!app) throw new NotFoundException('Application not found');
    const job = await this.db.getJobPostingById(app.job_id);
    if (!job || job.owner_user_id !== userId) throw new ForbiddenException('You can only update applications for your own job postings');

    const updated = await this.db.updateApplication(appId, payload.updates);
  ...
  async scheduleInterview(@Payload() payload: ScheduleInterviewDto) {
    const appId = payload?.app_id;
    const userId = payload?.user_id;
    const { interview_date, interview_time, interview_type, location, notes } = payload || ({} as any);
    if (!appId?.trim()) throw new BadRequestException('app_id is required');
    if (!userId?.trim()) throw new BadRequestException('user_id is required');
    if (!interview_date || !interview_time) {
      throw new BadRequestException('interview_date and interview_time are required');
    }

    const application = await this.db.getApplicationById(appId);
    if (!application) throw new NotFoundException('Application not found');
    const job = await this.db.getJobPostingById(application.job_id);
    if (!job || job.owner_user_id !== userId) throw new ForbiddenException('You can only schedule interviews for your own job postings');

    const updated = await this.db.updateApplication(appId, {
      interview_date,
      interview_time,
  ...
  async delete(@Payload() payload: DeleteApplicationDto) {
    ...
    if (!app) throw new NotFoundException('Application not found');
    const job = await this.db.getJobPostingById(app.job_id);
    if (!job || job.owner_user_id !== userId) throw new ForbiddenException('You can only delete applications for your own job postings');
    await this.db.updateApplication(appId, { status: 'DELETED' });
