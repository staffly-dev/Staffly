...
export const Logo = ({ className, uniColor }: LogoProps) => {
  return (
    <Image
      ...
      width={100}
      height={24}
      className={cn('h-10 w-auto', className, { 'invert': uniColor })}
      priority
    />
  )
}
...
export function LogoWithTitle({ className }: { className?: string }) {
  return (
    <div className={cn("flex items-center space-x-2", className)}>
      ...
      <span className="font-bold text-lg tracking-tight">Staffly</span>
    </div>
  );
}
...
export const LogoIcon = ({ className, uniColor }: LogoProps) => {
  return (
    <Image
      src="/imgs/logo.png"
      alt="Staffly Logo Icon"
      width={24}
      height={24}
      className={cn('w-5 h-5', className, { 'invert': uniColor })}
      priority
    />
  )
}
